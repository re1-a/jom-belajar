...
function toggleDarkMode() {
    ...
    if(typeof dailyChartInstance !== 'undefined' && dailyChartInstance) dailyChartInstance.update();
    if(typeof moduleChartInstance !== 'undefined' && moduleChartInstance) moduleChartInstance.update();
}
...
// Duplicate shuffleArray function removed by QA
...
function checkAnswer(selected, btnElement) {
    ...
    if (btnElement) {
        if (isCorrect) {
            ...
            if (mcqOptions) {
                const btns = mcqOptions.querySelectorAll('button');
                btns.forEach(b => {
...
function recordSession(percentage) {
    ...
    // Limit history to 100 records to prevent LocalStorage QuotaExceededError memory leak
    if (sainsHistory.length > 100) {
        sainsHistory = sainsHistory.slice(-100);
    }
...
function renderDragDrop(question, items, zones, correctMapping, explanation, image) {
    ...
    html += `<div class="draggable-items-wrapper" id="drag-items" ondrop="drop(event)" ondragover="allowDrop(event)" ondragenter="dragEnter(event)" ondragleave="dragLeave(event)">`;
    items.forEach(item => {
